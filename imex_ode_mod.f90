module imex_ode_mod
  use iso_c_binding, only: r8 => c_double, i4 => c_int
  implicit none

  private r8, i4
  private de, step, intrp

contains

  subroutine imex_ode (f, neqn, y, t, tout, relerr, abserr, iflag, work, iwork)
...
!
!    On the first call to ODE, the user must provide storage in the
!    calling
...
!    set to TOUT, and Y has been set to the approximate solution at
!    TOUT.
!
!    If IFLAG is 3, then the program noticed that RELERR or ABSERR was
!    too small; the output values of these variables are more
!    appropriate,
...
!    and the output value of Y is the solution at TOUT, which was
!    computed
...
!      real(r8) y(neqn)
!      real(r8) yp(neqn)
!    which accepts input values T and Y(1:NEQN), evaluates the right
!    hand
!    sides of the ODE, and stores the result in YP(1:NEQN).
!
!    Input, integer(i4) NEQN, the number of equations.
!
!    Input/output, real(r8) Y(NEQN), the current solution.
!
!    Input/output, real(r8) T, the current value of the
!    independent
!    variable.
!
!    Input, real(r8) TOUT, the desired value of T on output.
!
!    Input, real(r8) RELERR, ABSERR, the relative and absolute
!    error
!    tolerances.  At each step, the code requires
!      abs ( local error ) <= abs ( y ) * relerr + abserr
!    for each component of the local error and solution vectors.
!
!    Input/output, integer(i4) IFLAG, indicates the status of 
!    integration.  On input, IFLAG is normally 1 (or -1 in the special
!    case 
!    where TOUT is not to be exceeded.)  On normal output, IFLAG is 2.
!    Other 
!    output values are:
!    * 3, integration did not reach TOUT because the error tolerances
!      were too small.  But RELERR and ABSERR were increased
!      appropriately
!      for continuing;
!    * 4, integration did not reach TOUT because more than 500 steps
!    were taken;
...
!
!    Input/output, real(r8) WORK(100+21*NEQN), workspace.
!
!    Input/output, integer(i4) IWORK(5), workspace.
!
    implicit none
    ...
    real(r8) abserr
    external f
    integer(i4), parameter :: ialpha = 1
    integer(i4), parameter :: ibeta = 13
    integer(i4), parameter :: idelsn = 93
    integer(i4) iflag
    integer(i4), parameter :: ig = 62
    integer(i4), parameter :: ih = 89
    integer(i4), parameter :: ihold = 90
    integer(i4) ip
    integer(i4), parameter :: iphase = 75
    integer(i4) iphi
    integer(i4), parameter :: ipsi = 76
    integer(i4), parameter :: isig = 25
    integer(i4), parameter :: istart = 91
    integer(i4), parameter :: itold = 92
    integer(i4), parameter :: iv = 38
    integer(i4), parameter :: iw = 50
    integer(i4) iwt
    integer(i4), parameter :: ix = 88
    integer(i4) iyp
    integer(i4) iypout
    integer(i4), parameter :: iyy = 100
    integer(i4) iwork(5)
    logical nornd
    logical phase1
    real(r8) relerr
...
!    were
!         too small.
!         But RELERR and ABSERR were increased appropriately for
!         continuing;
!    * 4, integration did not reach TOUT because more than 500 steps
!    were taken;
!    * 5, integration did not reach TOUT because the equations appear to
...
!
!    Workspace, real(r8) YY(NEQN), used to hold old solution
!    data.
!
!    Input, real(r8) WT(NEQN), the error weight vector.
!
!    Workspace, real(r8) P(NEQN).
!
!    Workspace, real(r8) YP(NEQN), used to hold values of the
!    solution derivative.
!
!    Workspace, real(r8) YPOUT(NEQN), used to hold values of
!    the
!    solution derivative.
!
!    Workspace, real(r8) PHI(NEQN,16), contains divided
!    difference
!    information about the polynomial interpolant to the solution.
!
!    Workspace, real(r8) ALPHA(12), BETA(12), SIG(13).
!
!    Workspace, real(r8) V(12), W(12), G(13).
!
!    Input/output, logical PHASE1, indicates whether the program is in
!    the
!    first phase, when it always wants to increase the ODE method order.
!
!    Workspace, real(r8) PSI(12), contains information about
!    the polynomial interpolant to the solution.
!
!    Input/output, real(r8) X, a "working copy" of T, the
!    current value
!    of the independent variable, which is adjusted as the code attempts
!    to take a step.
!
!    Input/output, real(r8) H, the current stepsize.
!
!    Input/output, real(r8) HOLD, the last successful stepsize.
!
!    Input/output, logical START, is TRUE on input for the first step.
!    The program initializes data, and sets START to FALSE.
!
!    Input/output, real(r8) TOLD, the previous value of T.
!
!    Input/output, real(r8) DELSGN, the sign (+1 or -1) of
!    TOUT - T.
!
!    Input/output, integer(i4) NS, the number of steps taken
!    with 
!    stepsize H.
!
!    Input/output, logical NORND, ?
!
!    Input, integer(i4) K, the order of the current ODE method.
!
!    Input, integer(i4) KOLD, the order of the ODE method on
!    the 
!    previous step.
!
!    Input/output, integer(i4) ISNOLD, the previous value of
!    ISN, the 
!    sign of IFLAG.
!
!  Local parameters:
...
!  Discussion:
!
!    This routine integrates a system of first order ordinary
!    differential
!    equations one step, normally from x to x+h, using a modified
!    divided
!    difference form of the Adams PECE formulas.  Local extrapolation is
!    used to improve absolute stability and accuracy.  The code adjusts
!    its
!    order and step size to control the local error per unit step in a
!    generalized sense.  Special devices are included to control
!    roundoff
...
!    to be less than EPS for a successful step.  The array WT allows the
!    user
...
!    information needed to continue the integration, including the next
!    step
!    size H and the next order K, is returned with each step.  With the
!    exception of the step size, the error tolerance, and the weights,
!    none
!    of the parameters should be altered.  The array WT must be updated
!    after
!    each step to maintain relative error tests like those above.
!
!    Normally the integration is continued just beyond the desired
!    endpoint
!    and the solution interpolated there with subroutine INTRP.  If it
!    is
...
!
!    Changing the direction of integration, that is, the sign of H,
!    requires
!    the user to set START = TRUE before calling STEP again.  This is
!    the
!    only situation in which START should be altered.
!
!    A successful step: the subroutine returns after each successful
!    step with
!    START and CRASH both set to FALSE.  X represents the independent
!    variable
!    advanced by one step of length HOLD from its input value; Y has
!    been
!    updated to the solution vector at the new value of X.  All other
!    parameters
!    represent information corresponding to the new X needed to continue
!    the
!    integration.
!
!    Unsuccessful steps: when the error tolerance is too small, the
!    subroutine
!    returns without taking a step and sets CRASH to TRUE. An
!    appropriate step
!    size and error tolerance for continuing are estimated and all other
!    information is restored as upon input before returning.  To
!    continue
...
!  Parameters:
!
!    Input/output, real(r8) X, the value of the independent
!    variable.
!
!    Input/output, real(r8) Y(NEQN), the approximate solution
!    at the
...
!    Input, integer(i4) NEQN, the number of equations.
!
!    Input/output, real(r8) H, the suggested stepsize.
!
!    Input/output, real(r8) EPS, the local error tolerance.
!
!    Input, real(r8) WT(NEQN), the vector of error weights.
!
!    Input/output, logical START, is set to TRUE before the first step.
!    The program initializes data, and resets START to FALSE.
!
!    Input/output, real(r8) HOLD, the step size used on the
!    last
!    successful step.
!
!    Input/output, integer(i4) K, the appropriate order for the 
!    next step.
!
!    Input/output, integer(i4) KOLD, the order used on the last
!    successful step.
!
...
!
    ...
    fouru = 2.0D+00 * twou
!
!  Check if the step size or error tolerance is too small.  If this is
!  the
!  first step, initialize the PHI array and estimate a starting step
!  size.
!
...
!
    ...
    p5eps = 0.5D+00 * eps
!
!  If the error tolerance is too small, increase it to an acceptable
!  value.
!
...
!  If third consecutive failure, set order to one.  If the step fails
!  more
!  than three times, consider an optimal step size.  Double the error
!  tolerance and return if the estimated step size is too small for
!  machine
!  precision.
!
...
!
!  On third failure, set the order to one.  Thereafter, use optimal step
!  size.
!
       ifail = ifail + 1
       temp2 = 0.5D+00
...
!  Discussion:
!
!    The methods in STEP approximate the solution near X by a
!    polynomial.
...
!  Parameters:
!
!    Input, real(r8) X, the point where the solution has been
!    computed.
!
!    Input, real(r8) Y(NEQN), the computed solution at X.
!
!    Input, real(r8) XOUT, the point at which the solution is
!    desired.
!
!    Output, real(r8) YOUT(NEQN), the solution at XOUT.
!
!    Output, real(r8) YPOUT(NEQN), the derivative of the
!    solution
!    at XOUT.
!
!    Input, integer(i4) NEQN, the number of equations.
!
!    Input, integer(i4) KOLD, the order used for the last
!    successful step.
!
!    Input, real(r8) PHI(NEQN,16), contains information about
!    the
!    interpolating polynomial.
!
!    Input, real(r8) PSI(12), contains information about the
!    interpolating polynomial.
!
    implicit none
